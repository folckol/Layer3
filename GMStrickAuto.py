...
def Function(accs):
    ...
    for acc in accs:
        ...
        print(acc.id, acc.address, acc.date, acc.GMstrick, acc.XP)
        # input()
        ...
        readyTasks = acc.readyTasks

        if readyTasks:
            allTasks = readyTasks.split('|||')
            taskList = session.query(Task).filter(not_(Task.name.in_(allTasks))).all()
        else:
            taskList = session.query(Task).all()
        ...
        if len(taskList) > 0:
            ...
            for subTask in NowTask.bounty_steps:
                # print(subTask.bounty_step_id)
                try:
                    L3Account.BountyStep(subTask.bounty_step_id,
                                         json.loads(subTask.input_data) if subTask.input_data else None, AccID)
                except:
                    try:
                        L3Account.BountyStep(subTask.bounty_step_id,
                                             json.loads(subTask.input_data) if subTask.input_data else None,
                                             subTask.user_address_id)
                    except:
            ...
            for subTask in NowTask.bounty_claims:
                # print(subTask.TaskID)

                try:

                    result = L3Account.ClaimXP(subTask.TaskID)

                    # print(result)

                    if result[0]['error']['json']['message'] == 'Bounty has expired!' or result[0]['error']['json'][
                        'message'] == "You've already completed this quest!" or result[0]['error']['json'][
                        'message'] == 'You have not completed all steps!':
                        record = session.query(Record).filter(Record.id == acc.id).first()

                        if record.readyTasks:
                            RT = record.readyTasks.split('|||')
                            ...
                            record.readyTasks = NowTask.name
                            session.commit()
